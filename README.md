...
- **实现**：单文件 `server.py`（约 3 万行 Python，约 150 个工具，零 C++ 源码污染）

## 核心特点

- **完整覆盖 Qt 项目生命周期**：脚手架、构建、运行、测试、格式化、部署、签名、安装包一条龙
- **本地 FTS5 全文检索**：Qt 5.14.2 自带 6613 页文档建成索引，100 ms 内搜到答案
- **AI 友好的诊断信息**：`qt_build` 把编译器 / moc / uic / 链接器输出解析成结构化 JSON，并给出可执行的修复建议
- **完整 e2e 测试**：每个工具配套 pytest 验证（含 happy path + error path）
- **零样例代码污染**：所有 Qt 二进制（`*.exe` / `*.dll`）走 subprocess 调真实 Qt SDK，仓库内不引入任何 C++ 源码
- **路径沙箱保护**：所有工具强制路径必须在 sandbox 根下，跨边界访问直接拒绝
...
工具按职责分 19 个领域。所有工具都是 Python `async def`，签名见 `server.py`。

| 分类 | 代表工具 |
|---|---|
| 项目脚手架 | `qt_scaffold` / `qt_template_scaffold` / `qt_class_wizard` / `qt_pro_edit` |
| 构建 | `qt_build` / `qt_clean` / `qt_shadow_build_setup` / `qt_build_diagnostics` |
| 运行 | `qt_run` / `qt_run_trace` / `qt_kill_exe` / `qt_perf_budget` / `qt_perf_compare` |
| 测试 | `qt_test` / `qt_qml_test` / `qt_test_fuzz` / `qt_sanitizer_run` / `qt_test_coverage_diff` |
| 静态分析 | `qt_clazy_check` / `qt_cppcheck` / `qt_complexity_lint` / `qt_lint` / `qt_format_check` / `qt_analyze` |
| 文档与搜索 | `qt_docs_search` / `qt_docs_gen` / `qt_documentation_auto_fill` / `qt_translate` |
| 部署与签名 | `qt_deploy` / `qt_deploy_bundle` / `qt_signature` / `qt_signature_batch` / `qt_installer_gen` |
| 数据库 | `qt_db_seed` / `qt_db_validate` / `qt_db_dump` / `qt_db_perf_index` / `qt_db_schema_diff` |
| 网络 | `qt_network` / `qt_http_client_gen` / `qt_ftp_client_gen` |
| 多媒体 | `qt_multimedia_setup` |
| QML | `qt_qml_lint` / `qt_qml_perf_lint` / `qt_qml_component_gen` / `qt_qmlscene` |
| Qt 3D | `qt_qtquick_3d_setup` |
| C++ 重构 | `qt_modernize_qt5_to_qt6` / `qt_modernize_qt6_string_literal` |
| 信号与槽 | `qt_signal_slot_trace` / `qt_signal_disconnect_check` / `qt_qobject_invocation_count` / `qt_qobject_invoke_metadata` |
| Q_PROPERTY | `qt_property_browser` / `qt_hotreload_check` |
| 教学与示例 | `qt_cpp_tutorial_scaffold` / `qt_graphics_view_scaffold` |
| 主题与样式 | `qt_theme_gen` / `qt_qstyle_sheet_gen` |
| 游戏 / 棋牌 | `qt_scaffold --template chess_game / tictactoe_game / breakout_game / cards_game` |
| 其他实用 | `qt_achievement` / `qt_leaderboard_ui` / `qt_input_recorder` / `qt_replay` / `qt_save` 等 |
...
Claude / MCP 客户端
  ...
  server.py (FastMCP)
       │
       ├─ ~150 个 @mcp.tool 装饰的 async def qt_xxx(params) → str
       │
       ├─ 共享 helpers
...
│
├── docs/                  架构图 + 演示截图
├── examples/minimal/      hello Qt 5 分钟示例
├── tests/                 pytest 套件（每加一个工具配套一个 e2e 套件）
├── .github/               issue / PR 模板 + GitHub Actions CI
└── docs_data/             Qt 文档 FTS5 索引（51 MB，build_docs_index.py 生成）
```
